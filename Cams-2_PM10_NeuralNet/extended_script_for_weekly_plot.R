...
#15neurons performs best
wk_test_date<- seq(as.Date('2018-01-01'),as.Date('2018-12-30'),by = 7)
wk_test_date<-as.POSIXct(strptime(wk_test_date, format ="%Y-%m-%d" ))
wk_predicted15<-as.numeric(wk_predicted15)

wk_plot_df<-data.frame(wk_test_date,wk_actual_test,wk_predicted15)
colnames(wk_plot_df)<-c("date","actual","predicted")

w_10_2<- ggplot(data = wk_plot_df, aes(x = date)) +
  ...
  labs( y = "PM 10", title = "Predicted vs Observed values of PM 10 (weekly avg.) during 2018 in CAMS-2")

w_10_2

# Save an object to a file
saveRDS(w_10_2, file = "w_10_2.rds")
saveRDS(p_10_2, file = "p_10_2.rds")
