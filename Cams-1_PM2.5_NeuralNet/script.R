data <- read.csv("CAMS1-SANGSAD all date.csv")
summary(data)
data<-data[366:2191,]
summary(data)
data<-data[-c(1,9)]
...
p_2.5_1<- ggplot(data = plot_df, aes(x = date)) +
  ...
  labs( y = "PM 2.5", title = "Predicted vs Observed values of PM 2.5 during 2018 in CAMS-1") 

p_2.5_1



R2 <- caret::postResample(pred = predicted7, obs = actual_test)
r2 <- ggplot(plot_df, aes(actual, predicted))
r2 <- r2 + geom_point(alpha = .5)
r2 <- r2 + annotate(geom = "text", x=175,y=100,  label = paste("R**2 = ", round(R2[2], 3)))
r2 <- r2 + labs(x = "Actual", y = "Predicted", title = "Correlation Plot (CAMS-1)")
