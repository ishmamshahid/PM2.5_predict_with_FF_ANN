...
#5neurons performs best
wk_test_date<- seq(as.Date('2018-01-01'),as.Date('2018-12-30'),by = 7)
wk_test_date<-as.POSIXct(strptime(wk_test_date, format ="%Y-%m-%d" ))
wk_predicted5<-as.numeric(wk_predicted5)

wk_plot_df<-data.frame(wk_test_date,wk_actual_test,wk_predicted5)
colnames(wk_plot_df)<-c("date","actual","predicted")

w_2.5_1<- ggplot(data = wk_plot_df, aes(x = date)) +
  ...
  labs( y = "PM 2.5", title = "Predicted vs Observed values of PM 2.5 (weekly avg.) during 2018 in CAMS-1") 

w_2.5_1

# Save an object to a file
saveRDS(w_2.5_1, file = "w_2.5_3.rds")
saveRDS(wk_metrics_df, file = "wk_metrics_df.rds")

saveRDS(r2, file = "r2_1.rds")
