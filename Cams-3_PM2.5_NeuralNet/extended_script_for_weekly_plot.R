...
wk_predicted3 <- wk_model_results_3$net.result
wk_predicted5 <- wk_model_results_5$net.result
wk_predicted7 <- wk_model_results_7$net.result
wk_predicted9 <- wk_model_results_9$net.result
wk_predicted12 <- wk_model_results_12$net.result
wk_predicted15 <- wk_model_results_15$net.result
wk_predicted20 <- wk_model_results_20$net.result
...
#12neurons performs best
wk_test_date<- seq(as.Date('2018-01-01'),as.Date('2018-12-30'),by = 7)
wk_test_date<-as.POSIXct(strptime(wk_test_date, format ="%Y-%m-%d" ))
wk_predicted12<-as.numeric(wk_predicted12)

wk_plot_df<-data.frame(wk_test_date,wk_actual_test,wk_predicted12)
colnames(wk_plot_df)<-c("date","actual","predicted")

w_2.5_3<- ggplot(data = wk_plot_df, aes(x = date)) +
  ...
  labs(y = "PM 2.5", title = "Predicted vs Observed values of PM 2.5 during 2018 in CAMS-3")  

w_2.5_3

# Save an object to a file
saveRDS(w_2.5_3, file = "w_2.5_3.rds")
saveRDS(wk_metrics_df, file = "wk_metrics_df.rds")


saveRDS(p_2.5_3, file = "p_2.5_3.rds")
